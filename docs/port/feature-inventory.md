...
- Discovery detail presented inside animated modal (`DiscoveryModalContext.tsx` + `components/custom/DiscoveryDetails`), with inline Markdown rendering, share token data, map deep links, delete controls, and quick action buttons to capture/upload again. *(Swift port: measured-card hero overlay with cached image snapshot, gradient stack, share/map controls, Markdown body, and voiceover playback (inline + persistent player) are implemented; delete controls, quick action buttons, and swipe dismissal remain pending.)*
